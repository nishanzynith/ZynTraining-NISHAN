        ...
        dataitem(Expense_Tracker; "Expense Entry")
        {
            ...
            begin
                if (Expense_Tracker.Date >= StartDate) and (Expense_Tracker.Date <= EndDate) then begin
                    if (CategoryFilter = '') or (Expense_Tracker.Category = CategoryFilter) then begin
                        ExcelBuffer.AddColumn(Expense_Tracker.Category, FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                        ExcelBuffer.AddColumn(Expense_Tracker.Date, FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Date);
                        ExcelBuffer.AddColumn(Expense_Tracker.Amount, FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                        ExcelBuffer.NewRow();

                        TotalAmount += Expense_Tracker.Amount;
                        RecordsFound := true;
                    end;
                end;
            end;
